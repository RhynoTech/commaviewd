...
for script in "$RUNNER" "$PIPELINE" "$INTERFACE_GUARD" "$INTERFACE_GUARD_CONTRACT" "$BINARY_CONTRACT" "$HUD_LITE_CI_CONTRACT" "$HUD_LITE_PATCH_CONTRACT" "$LOCAL_DISCOVERY_CONTRACT" "$BUNDLE"; do
