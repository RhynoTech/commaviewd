...
runtime_debug_policy_contract_main() {
  ...
  assert_contains_fixed 'std::thread telemetry_thread' "$bridge_cpp" 'direct v2 cutover should run telemetry in a dedicated thread'
  assert_contains_fixed 'telemetry_thread.join()' "$bridge_cpp" 'telemetry thread should be joined on disconnect'
  assert_contains_fixed 'SubSocket::create(ctx, service_name, "127.0.0.1", true, true, segment_size);' "$bridge_cpp" 'direct v2 subscribers should use conflated raw subscriptions'
