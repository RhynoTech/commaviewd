...
runtime_debug_policy_contract_main() {
  ...
  assert_contains_fixed '{"wideRoadCameraState", {ServiceMode::Pass, 0}}' "$policy_header" 'wideRoadCameraState telemetry should default to pass'
  ...
  assert_contains_fixed 'udp_telemetry_snapshot_loop' "$bridge_cpp" 'direct ui socket telemetry should publish via UDP snapshot loop'
  assert_not_contains_fixed 'telemetry_thread.join()' "$bridge_cpp" 'TCP telemetry thread should be removed'
