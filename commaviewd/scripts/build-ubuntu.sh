...
UDP_VIDEO_PROTOCOL_SRC="$ROOT/src/udp_video_protocol.cpp"
UDP_VIDEO_PACKETIZER_SRC="$ROOT/src/udp_video_packetizer.cpp"
UDP_VIDEO_REPAIR_CACHE_SRC="$ROOT/src/udp_video_repair_cache.cpp"
UDP_VIDEO_SENDER_SRC="$ROOT/src/udp_video_sender.cpp"
UDP_TELEMETRY_SNAPSHOT_SRC="$ROOT/src/udp_telemetry_snapshot.cpp"
...
for f in \
  ...
  "$CONTROL_POLICY_SRC" "$VIDEO_ROUTER_SRC" "$VIDEO_TRANSPORT_POLICY_SRC" \
  "$UDP_VIDEO_PROTOCOL_SRC" "$UDP_VIDEO_PACKETIZER_SRC" "$UDP_VIDEO_REPAIR_CACHE_SRC" "$UDP_VIDEO_SENDER_SRC" "$UDP_TELEMETRY_SNAPSHOT_SRC" \
  "$RUNTIME_VIDEO_SEND_ACCOUNTING_SRC" "$TELEMETRY_JSON_SRC" "$TELEMETRY_STATS_SRC" \
...
COMMON_SRCS=(
  ...
  "$VIDEO_ROUTER_SRC" "$VIDEO_TRANSPORT_POLICY_SRC"
  "$UDP_VIDEO_PROTOCOL_SRC" "$UDP_VIDEO_PACKETIZER_SRC" "$UDP_VIDEO_REPAIR_CACHE_SRC" "$UDP_VIDEO_SENDER_SRC" "$UDP_TELEMETRY_SNAPSHOT_SRC"
  "$RUNTIME_VIDEO_SEND_ACCOUNTING_SRC" "$TELEMETRY_JSON_SRC" "$TELEMETRY_STATS_SRC"
  "$OP_ROOT/cereal/messaging/socketmaster.cc"
  "$MSGQ_SOURCE"
