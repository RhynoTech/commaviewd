...
for f in \
  ...
  "$CONTROL_POLICY_SRC" "$VIDEO_ROUTER_SRC" "$VIDEO_TRANSPORT_POLICY_SRC" "$TELEMETRY_JSON_SRC" "$TELEMETRY_STATS_SRC" \
...
COMMON_SRCS=(
  ...
  "$VIDEO_ROUTER_SRC" "$VIDEO_TRANSPORT_POLICY_SRC" "$TELEMETRY_JSON_SRC" "$TELEMETRY_STATS_SRC"
  "$OP_ROOT/cereal/messaging/socketmaster.cc"
  "$MSGQ_SOURCE"
  "$OP_ROOT/msgq_repo/msgq/event.cc" "$OP_ROOT/msgq_repo/msgq/impl_fake.cc"
